    ...
    def event_loop(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                if not self.hop:
                    if self.board.location(self.mouse_pos).occupant is not None and self.board.location(
                        ...
                        if self.mouse_pos not in adjacent(self.selected_piece):
                            self.board.remove_piece(
                                (int(self.selected_piece[0] + (self.mouse_pos[0] - self.selected_piece[0]) / 2),
                                 int(self.selected_piece[1] + (self.mouse_pos[1] - self.selected_piece[1]) / 2)))
...
def adjacent(pos):
    ...
    return [rel(NORTHWEST, (x, y)), rel(NORTHEAST, (x, y)), rel(SOUTHWEST, (x, y)),
            rel(SOUTHEAST, (x, y))]
    ...
    def blind_legal_moves(self, pos):
        ...
        if self.matrix[x][y].occupant != None:
            ...
            if self.matrix[x][y].occupant.king == False and self.matrix[x][y].occupant.color == BLUE:
                blind_legal_moves = [rel(NORTHWEST, (x, y)), rel(NORTHEAST, (x, y))]

            elif self.matrix[x][y].occupant.king == False and self.matrix[x][y].occupant.color == RED:
                blind_legal_moves = [rel(SOUTHWEST, (x, y)), rel(SOUTHEAST, (x, y))]

            else:
                blind_legal_moves = [rel(NORTHWEST, (x, y)), rel(NORTHEAST, (x, y)),
                                     rel(SOUTHWEST, (x, y)), rel(SOUTHEAST, (x, y))]
